...
export const checkCreateSavings = () => {
    return [
        ...
        body('start_date').isDate().withMessage('Please, provide a valid date  in format YYYY/MM/DD')
            .custom(customStart).withMessage('start_date cannot be less than today'),
        body('end_date').isDate().withMessage('Please, provide a valid date in formar YYYY/MM/DD'),
        body('frequency').custom(customFreqCheck).withMessage(`Frequency can only be :${ALL_FREQUENCY.toString()} `)
...
export const checkEditSavings = () => {
    return [
        ...
        body('end_date').isDate().withMessage('Please, provide a valid date in formar YYYY/MM/DD')
            .custom(customStart).withMessage('end_date cannot be less than today'),
        body('frequency').custom(customFreqCheck).withMessage(`Frequency can only be :${ALL_FREQUENCY.toString()} `)
    ]
}
...
const customFreqCheck = value => {
    if (ALL_FREQUENCY.includes(value))
        return true
    return false
}
